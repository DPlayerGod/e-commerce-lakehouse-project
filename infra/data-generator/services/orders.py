"""Orders Service - Generate order → payment → shipment events."""
...
class OrderService:
    """Generate realistic order transaction chains."""
    ...
    def emit(self) -> dict | None:
        """
        Emit one complete order transaction (order → payment → shipment).
        
        Returns:
            Shipment dict if created, else None (for delivery event scheduling)
        """
        base_ts = now_ms()
        order_id = rid("ord")
        ...
        if random.random() < self.cfg.p_order_has_payment:
            ...
            self.publisher.publish(
                ...
                key=pay["payment_id"],
                value=self.payment_encoder.encode(pay),
                headers={"trace_id": trace_id, "entity": "payment", "source": "fakegen"},
            )

            # Step 3: Create SHIPMENT (60% probability if SETTLED)
            if pay["status"] == "SETTLED" and random.random() < self.cfg.p_order_has_shipment:
                ...
                self.publisher.publish(
                    ...
                    key=shp["shipment_id"],
                    value=self.shipment_encoder.encode(shp),
                    headers={"trace_id": trace_id, "entity": "shipment", "source": "fakegen"},
                )
                
                # Return shipment data for delivery event scheduling
                return {
